...
type PodRepository interface {
	...
	DeletePod(context.Context, authorization.Info, repositories.AppRecord, string, string) error
}
...
func (h *App) restartInstance(r *http.Request) (*routing.Response, error) {
	...
	if process.DesiredInstances <= instance {
		msg := fmt.Sprintf("Instance %d of process %s", instance, processType)
		err = errors.New(msg + "not found")
		return nil, apierrors.LogAndReturn(logger,
			apierrors.NewNotFoundError(err, msg), "Instance not found", "AppGUID", appGUID, "InstanceID", instanceID, "Process", process)
	}
